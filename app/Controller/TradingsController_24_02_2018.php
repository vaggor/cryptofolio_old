...
App::import('Model', 'Binance');
  ...
  public $components = array('Session', 'Email','Auth');
  	...
  	public function dashboard(){
  		$sess = $this->Auth->user();
		$user_id = $sess['id'];
		$active_trades = $this->RobotTrade->countAllActiveTradeByUser($user_id);
		$close_trades = $this->RobotTrade->countAllClosedTradeByUser($user_id);
		$total_trades = $active_trades + $close_trades;
		$robots = $this->Robot->countRobotsByUser($user_id);
		$profits = $this->RobotTrade->getProfitByUserWithoutRobot($user_id);
		$coins = $this->RobotTradingCoin->countActiveCoinsByUser($user_id);
		$this->set(compact('active_trades','close_trades','total_trades','robots','profits','coins'));
  	}
