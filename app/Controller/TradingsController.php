	...
	public function buy(){
		...
		if($sess['balance'] <= 0){
  		...
  		if(!empty($this->data)){
  			...
  			$user_id = $sess['id'];
  			if(empty($data['RobotTrade']['threshold'])){
  				$threshold = 1;
  			}
  			else{
  				$threshold = str_replace('%', '', $data['RobotTrade']['threshold']);
  			}

  			if(empty($data['RobotTrade']['stop_loss'])){
  				$stop_loss = '-1000';
  			}
  			else{
  				$stop_loss = str_replace('%', '', $data['RobotTrade']['stop_loss']);
  				if($stop_loss > 0){
  					$this->Session->setFlash('Stop loss should be negative','default',array('class'=>'error1'));
  					$this->redirect('buy');
  				}
  			}
  			if($data['RobotTrade']['exchange_id'] == 2){
  				$trade_pair = str_replace('/', '', $trade_pair[0]['TradingPair']['pair']);
  				$order_resp = $this->saveBinanceData($api_key,$api_secret,$trade_pair,$trading_pair_id,$quantity,$exchange_id,$robot_id,$user_id,$threshold,$stop_loss);
  	...
  	public function edit_profit_loss($id=null){	
		if(empty($this->data)){
			//print_r($id);exit;
			$sess = $this->Auth->user();
			$user_id = $sess['id'];
		...
		elseif (!empty($this->data)) {
			...
			if ($this->RobotTrade->save($data)) {
				$this->Session->setFlash('Updated Successfully.');
				$this->redirect(array('action' => 'all_trades'));
			}
		}
	}
  	...
  	public function saveBinanceData($api_key,$api_secret,$trade_pair,$trading_pair_id,$quantity,$exchange_id,$robot_id,$user_id,$threshold,$stop_loss){
  		$sess = $this->Auth->user();
  		...
  		if(@$order['status'] == 'FILLED'){
		  	...
		  	$data['RobotTrade']['threshold'] = $data['RobotTrade']['buy_perc_chg'] + $threshold;
		  	$data['RobotTrade']['stop_loss'] = $stop_loss;
...
/****************************************************** Robot Trading Coins ***********************************************************************/
	...
	public function new_trading_coin(){
  		...
  		if(!empty($this->data)){
  			...
  			if(empty($data['RobotTradingCoin']['threshold'])){
  				$threshold = 1;
  			}
  			else{
  				$threshold = str_replace('%', '', $data['RobotTradingCoin']['threshold']);
  			}

  			if(empty($data['RobotTradingCoin']['stop_loss'])){
  				$stop_loss = '-1000';
  			}
  			else{
  				$stop_loss = str_replace('%', '', $data['RobotTradingCoin']['stop_loss']);
  				if($stop_loss > 0){
  					$this->Session->setFlash('Stop loss should be negative','default',array('class'=>'error1'));
  					$this->redirect('new_trading_coin');
  				}
  			}

  			$data['RobotTradingCoin']['threshold'] = $threshold;
  			$data['RobotTradingCoin']['stop_loss'] = $stop_loss;
  	...
  	public function edit_trading_coin($id=null){
  		...
  		elseif(!empty($this->data)){
  			...
  			if(empty($data['RobotTradingCoin']['stop_loss'])){
  				...
  				if($stop_loss > 0){
  					$this->Session->setFlash('Stop loss should be negative','default',array('class'=>'error1'));
  					$this->redirect('edit_trading_coin/'.$data['RobotTradingCoin']['id']);
  				}
  			}
